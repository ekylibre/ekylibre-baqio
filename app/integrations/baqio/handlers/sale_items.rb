    ...
    class SaleItems
      ...
      private
        ...
        def create_sale_items(sale, order, order_line_not_deleted)
          ...
          if variant.blank?
            pnv_service = Baqio::Handlers::ProductNatureVariants.new(vendor: @vendor)
        ...
        def find_baqio_tax_to_eky(order_line_not_deleted, order)
          ...
          elsif Baqio::Handlers::CountryTaxes::EU_CT_CODE.include?(order[:accounting_tax]) && !order[:tax_lines].present?
        ...
        def find_or_create_baqio_country_tax(tax_line, order)
          ...
          unless tax
            ...
            if order[:accounting_tax] == 'eu' || Baqio::Handlers::CountryTaxes::EU_CT_CODE.include?(order[:accounting_tax])
              # for person
              if order[:customer][:billing_information][:legal_form].blank? && order[:customer][:billing_information][:vat_number].blank?
                tax = Baqio::Handlers::CountryTaxes.new(vendor: @vendor).find_or_create_specific_tax(country_tax_id)
              # for enterprise
              else
